...
{{< video src="https://www.youtube.com/watch?v=UNJximu2suM" >}}
...
-   Note the use of the greater than or equal to `>=` and less than or equal signs `<=` to specify which rows we want to keep in the data. We want rows that are in dates after January 1, 2019 and (`&`) on or before April 1, 2022.

```{r}
covidsub <- subset(covid, Dates >= as.Date("2019-01-01") &
                     Dates <= as.Date("2022-04-01"))
```
...
axisdates <- seq(as.Date("2019-01-01"), as.Date("2022-04-01"), by="month")
...
-   To do so, we will draw a vertical line at March 16, 2020
