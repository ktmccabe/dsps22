...
-   `term`: is the year of the SC (1991-2020 except for 2005),
...
```{r, include=F}
justices <- read.csv("data/justices20.csv", stringsAsFactors = T)
```

```{r, eval=F}
justices <- read.csv("justices20.csv", stringsAsFactors = T)

## alternative
justices <- read.csv("https://raw.githubusercontent.com/ktmccabe/teachingdata/main/justices20.csv", stringsAsFactors = T)
```
...
![[FiveThirtyEight](https://fivethirtyeight.com/features/how-ketanji-brown-jackson-could-change-the-supreme-court/)](images/sccourtkj.png)
